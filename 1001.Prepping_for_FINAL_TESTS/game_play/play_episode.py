...
class Episode:
    ...
    def play_episode(self, view_game = False):
        ...
        metrics = {}
        for met in ['ep_id', 'action','obs','reward','done','policy','n_step_returns','v','rdn_beta','exp_r','actor_id','stnum']: 
            metrics[met] = []
        ...
        while True:
            ...
            if self.cfg.exploration_type == 'episodic':
                self.state_vectors.append(copy.deepcopy(self.model.close_state_projection_obs(o[:,-3:])))
            ...
            if self.cfg.running_reward_in_obs:
                self.obs_deque.append(np.expand_dims(np.zeros_like(obs[0]),0) + (self.running_reward))
        ...
        if self.cfg.use_two_heads:
            if self.test_mode:
                ...
                if self.score_log.best_adv > 0 and np.random.uniform(0,1) < self.cfg.follow_better_policy:
                    self.actor_policy = 1
                    if np.random.uniform(0,5) < 1 and not self.resampling:
