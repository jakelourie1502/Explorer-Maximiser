...
def loss_func_r(r, true_values, dones_k, weights,cfg):
    """
...
def loss_func_v(v, true_values, dones_k,weights,cfg):
...
def loss_func_expV(v, true_values, dones_k,cfg):
...
def loss_func_p(p, true_policy, dones_k,weights,cfg):
...
def loss_func_proj(stopped_proj, w_grad_head, dones,cfg):
    #L1 loss
...
def loss_func_future_nov(nov, true_unclaimed_nov, dones_k, weights,cfg):
...
class RDN_loss:
    def __init__(self,cfg):
        ...
        for c in range(cfg.training.k):
            ...
            self.dekis[str(c+1)] = deque([0],10000)
            self.deki_stats[str(c+1)].mu = 0
            self.deki_stats[str(c+1)].sigma = 1
        self.update_log = []
        self.new_ep_expV_deki = deque([0],10000)
        self.new_ep_mu = 0

    def tanchy(self,x):
        sng = torch.sign(x)
        absx = torch.abs(x)**2
        return 1.5*torch.tanh(1.5*absx*sng)

    def evaluate(self,rdn, pred,k):
        if self.cfg.RND_loss == 'cosine':
    ...
    def train(self, rdn, pred,dones, weights, k):
        ...
        return torch.mean(weights*x) #so we return the score we would get in the rdn_eval

    def update(self):
        for c in range(self.cfg.training.k):
