        ...
        while algo.frame_count < algo.cfg.total_frames:
            ...
            if algo.training_flags.train_flag:
                ...
                for k in range(algo.K):
                    ...
                    if algo.cfg.exploration_type == 'episodic' and algo.training_step_counter % 2 == 0:
                        o = torch.tensor(np.array([algo.replay_buffer.obs[x+k+1] for x in indices])).float()
                        obs_next = o[:, -3:] #unstacked observation
                        projected_state = algo.ExpMaxTrain.close_state_projection(s)
                        projected_obs = algo.ExpMaxTrain.close_state_projection_obs(obs_next)
                        predicted_action = algo.ExpMaxTrain.close_state_classifer(torch.cat((projected_state, projected_obs),1))
                        next_actions = torch.tensor(action_index.reshape(-1,1))
                        next_actions_hot = torch.nn.functional.one_hot(next_actions, algo.cfg.actions_size).squeeze(1)
                        # loss_action_pred = -torch.mean(next_actions_hot * torch.log(predicted_action +1e-5))
                        ...
                        last_state_proj = projected_state
