...
from global_settings import  exploration_strategy, log_states, exp_v_support, env_size, workers, epochs, device,  actions_size, value_only, training_params, batches_per_train, batch_size, loading_in, start_epoch, train_start_batch_multiple, prioritised_replay, value_support
import torch    
from utils import global_expl, get_epoch_params, vector2d_to_tensor_for_model
...
def Play_Episode_Wrapper():
    with torch.no_grad():
        ...
        metrics, rew,ep_explr_log, analysis = ep.play_episode(rdn_loss_obj, pick_best, epoch = e)
        analysis_log.append(analysis) #so we end with a list of 7 x S arrays
...
for e in range(start_epoch, epochs):
    ...
    if len(replay_buffer.action_log) > batch_size*train_start_batch_multiple:
        ...
        for batch in range(batches_per_train):
            ...
            for s in state:
                with torch.no_grad():
                    episode = Episode(jake_zero_self_play, epsilon=0)
                    episode.actor = 'batman';   episode.actor_id = 0
                    mcts = MCTS(jake_zero_self_play.rdn_obj, episode, epoch = e, pick_best=False)
                    root_node = Node('null')
        ...
        if e % 10 ==0:
            ...
            analysis_save_file = np.concatenate((analysis_log),0)
            ...
            if e % 10 == 0:
                ...
                for _ in range(50):
                    with torch.no_grad():
                        ...
                        _, rew,_, _= ep.play_episode(rdn_loss_obj, True, epoch = max(51,e))
                        rew_test.append(rew)
