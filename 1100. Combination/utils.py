...
class Scores:
    def __init__(self,cfg):
        self.cfg = cfg
        self.rdns = np.round(np.linspace(*cfg.rdn_beta),3)
        self.scores = {}
    ...
    def update(self):
        ...
        max_prob = np.max(self.probs) + 1e-4
        min_prob = np.min(self.probs) - 1e-4
        for i in range(len(self.probs)):
            self.probs[i] = ((self.probs[i]-min_prob) / (max_prob - min_prob)) + (1/len(self.rdns))
        total_probs = np.sum(self.probs)
        for i in range(len(self.probs)):
            self.probs[i] /= total_probs
